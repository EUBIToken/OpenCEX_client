				...
				if(cdata.length != 0){
					...
					if(cdata.length > 1){
						...
						for(let i = 1; (i < cdata.length) && (cdata2.length < 60); i++){
							...
							const distance = parseInt((new BigInt(cdata[i])).sub(prevtime).div(span).toString());
							prev = cdata[i];
							for(let c = 0; c < distance && cdata2.length < 60; ){
								cdata2.push({x: prevtime.add((new BigInt(++c)).mul(span)).toString(), o: prev.c, h: prev.c, l: prev.c, c: prev.c});
							}
							if(cdata2.length < 60){
								cdata2.push(prev);
							}
